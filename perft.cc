...
Nodes perft(const Board &board, const Depth depth)
{
...
int main(int argc, char *argv[])
{
	...
	if ((bench && (divide || upto || verify)) || (upto && (divide || bench || verify)) || (verify && (divide || bench || upto)))
	{
		fmt::print("Incorrect usage: bench, divide, upto, verify are mutually exclusive options\n");
		return 0;
	}

	Depth depth = result.count("depth") ? result["depth"].as<std::uint16_t>() : 0;
	std::string fen;

	if (result.count("fen"))
	{
		fen = result["fen"].as<std::string>();
		...
		Board board;
		int status = parse_fen(board, fen);
		...
		if (status == 0)
		{
			...
			for (Depth d = (upto ? 1 : depth); d <= depth; ++d)
			{
				...
				if (divide)
				{
					fmt::print(
						"\n{} nodes\n{} ms\n{:.0f} nodes/sec\n",
						nodes, duration_cast<Milliseconds>(dt).count(), (1e6 * nodes) / dt.count()
					);
				}
				else
				{
					fmt::print(
						"{: <6} {: <12} {: <12} {:.0f}\n",
						d, nodes, duration_cast<Milliseconds>(dt).count(),
						(1e6 * nodes) / dt.count()
					);
				}
			}
		...
		{
			fmt::print(
				"Error: FEN parser returned non-zero code {} when parsing '{}'\n",
				status, fen
			);
	...
	else if (bench)
	{
		...
		for (const auto &name_fen_depth : PredefinedFENs)
		{
			...
			fmt::print(
				"{: <10} {: <6} {: <12} {: <12} {:.0f}\n",
				name_fen_depth.name, name_fen_depth.depth,
				nodes, duration_cast<Milliseconds>(dt).count(),
				(1e6 * nodes) / dt.count()
			);
		}
		...
		fmt::print(
			"{: <10} {: <6} {: <12} {: <12} {:.0f}\n",
			"total/avg", '-',
			total_nodes, total_time.count(),
			(1e3 * total_nodes) / total_time.count()
		);
	}
	else if (verify)
	{
